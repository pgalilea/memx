# Source: https://github.com/python/cpython/blob/main/Lib/uuid.py
# type: ignore
...
class UUID:
    ...
    def __repr__(self):
        return "%s(%r)" % (self.__class__.__name__, str(self))  # noqa
...
def _arp_getnode():
    ...
    mac = _find_mac_near_keyword("arp", "-an", [os.fsencode("(%s)" % ip_addr)], lambda i: i + 2)  # noqa: UP031
...
def getnode():
    ...
    for getter in _GETTERS + [_random_getnode]:
        ...
        if (_node is not None) and (0 <= _node < (1 << 48)):
            return _node
    assert False, f"_random_getnode() returned invalid value: {_node}"  # noqa: B011
